...
from validrig.qms.render import (
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    print(f"validrig {ENGINE_VERSION} — pack {pack.manifest.id} v{pack.manifest.version}")
...
def _cmd_lint(args: argparse.Namespace) -> int:
    from validrig.authoring.lint import has_errors, lint_pack
...
def _cmd_ingest(args: argparse.Namespace) -> int:
    from validrig.ingest.pipeline import INGEST_GUARANTEE, IngestError, ingest_case
    from validrig.pathsafe import require_safe_id
...
def _cmd_new(args: argparse.Namespace) -> int:
    ...
    print(f"  try: rig run {root} --battery smoke --out ./runs --seed 1")
    return 0


def _cmd_monitor(args: argparse.Namespace) -> int:
    from validrig.monitoring import build_snapshot, evaluate_drift, load_events
    from validrig.qms.pms import build_aims_event, build_pms_report
    from validrig.qms.render import render_pms_report_md
...
def _cmd_ui(args: argparse.Namespace) -> int:
    try:
        ...
        print(
            "The review UI needs the 'ui' extra: pip install 'validrig[ui]'",
            file=sys.stderr,
        )
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="rig", description="validrig — the DearAuditor Eval engine")
    parser.add_argument("--version", action="version", version=f"validrig {ENGINE_VERSION}")
