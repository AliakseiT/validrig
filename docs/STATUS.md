...
.venv/bin/pytest -q                                   # 136 tests, ~3s
...
| **M4 agent SUTs** | 🟡 core done | ✅ deterministic **tool mocks** (keyed by case/tool/args-hash, pack content → pack_hash), a **fake agent** (kind=`agent`) that emits a `Trace`, **process rubrics** (`RubricItem.target=trace`) graded against the trace and kept separate from output, and the `demo-agent` pack proving **right-answer-wrong-process** (agent-lazy reports the finding but skips the tool → output passes, process fails, acceptance FAIL). Process rubric is N/A (not a failure) for non-agent SUTs. ❌ deferred: agent perturbation axes (tool-availability ablation, tool-response perturbation) — second increment |
